...
const Sports = () => {
  ...
  const storedData = JSON.parse(
    localStorage.getItem(
      `@@auth0spajs@@::${process.env.REACT_APP_AUTH0_CLIENTID}::${process.env.REACT_APP_AUTH0_AUDIENCE}::openid profile email offline_access`
    )
  );
  ...
  const handleDelete = () => {
    axios
      .delete(`${baseUrl}/sport/${currentSport.id}`, {
        headers: headers,
      })
      .then((response) => {
  ...
  const handleSave = () => {
    if (currentSport.id) {
      axios
        .patch(`${baseUrl}/sport/${currentSport.id}`, currentSport, {
          headers: headers,
        })
        .then((response) => {
          setSports(
            sports.map((sport) =>
              sport.id === currentSport.id ? currentSport : sport
            )
          );
        });
    } else {
  ...
  return (
    <div>
              ...
              <td style={{ textAlign: "center" }}>
                {isAuthenticated && user.sub === sport.admin && (
                  <>
                    ...
                    <Button onClick={() => openDeleteDialog(sport)}>
                      Delete
                    </Button>
                  </>
                )}
      ...
      <Dialog open={open} onClose={closeDialog}>
        <DialogTitle>
          {currentSport.id ? "Edit Sport" : "New Sport"}
        </DialogTitle>
        <DialogContent>
          <FormControl fullWidth style={{ marginTop: "10px" }}>
            ...
            {
              // Check if filtered sports enum is empty
              sportEnums.filter(
                (sportEnum) => !sports.some((sport) => sport.name === sportEnum)
              ).length === 0 ? (
                <Typography variant="body2">
                  All available sports added
                </Typography>
              ) : (
                <Select
                ...
                >
                  {sportEnums
                    .filter(
                      (sportEnum) =>
                        !sports.some((sport) => sport.name === sportEnum)
                    )
