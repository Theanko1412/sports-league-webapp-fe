...
const Leagues = () => {
    ...
    if (isAuthenticated) {
      if (currentLeague.sportName) {
        setFilteredTeams(
          allTeams.filter(
            (team) =>
              team.sportName === currentLeague.sportName &&
              team.leagueId === null
          )
        );
      } else {
  ...
  const handleSave = async () => {
    try {
      ...
      if (updatedLeague.id) {
        await axios
          .patch(`${baseUrl}/league/${updatedLeague.id}`, updatedLeague, {
            headers: headers,
          })
          .then((response) => {
            setLeagues(
        ...
        await axios
          .post(`${baseUrl}/league`, updatedLeague, {
            headers: headers,
          })
          .then((response) => {
  ...
  const handleDelete = () => {
    axios
      .delete(`${baseUrl}/league/${currentLeague.id}`, {
        headers: headers,
      })
      .then((response) => {
  ...
  return (
    <div>
            ...
            <tr key={league.id}>
              ...
              <td style={{ textAlign: "center" }}>
                {isAuthenticated && user.sub === league.admin && (
                  <>
                    <Button disabled onClick={() => openDialog(league)}>
                      Edit
                    </Button>
                    <Button onClick={() => openDeleteDialog(league)}>
                      Delete
                    </Button>
                  </>
                )}
      ...
      <Dialog open={open} onClose={closeDialog}>
        <DialogTitle>
          {currentLeague.id ? "Edit League" : "New League"}
        </DialogTitle>
        <DialogContent>
          <TextField
            name="name"
          ...
          <FormControl fullWidth>
            ...
            {filteredTeams.length === 0 ? (
              <Typography variant="body2">
                All teams for this sport already in league
              </Typography>
            ) : (
              <Select
